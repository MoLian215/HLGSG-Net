...
from model.MODEL import Mulit_layer_Encoder, GCN_Vertex
...
def train_model():
    ...
    MLE_path = './model_weight/pretrain_ver.pth'
    ...
    feat_set_t1, feat_set_t2 = [], []

    for idx in range(obj_num):
        obj_feat_t1 = node_set_t1[idx]
        obj_feat_t2 = node_set_t2[idx]

        mean_feat_t1 = np.mean(obj_feat_t1, axis=0)
        mean_feat_t2 = np.mean(obj_feat_t2, axis=0)

        feat_set_t1.append(mean_feat_t1)
        feat_set_t2.append(mean_feat_t2)

    feat_set_t1 = np.array(feat_set_t1)
    feat_set_t2 = np.array(feat_set_t2)

    dist_set_t1 = cal_nonlocal_dist(feat_set_t1, args.band_width_t1)
    dist_set_t2 = cal_nonlocal_dist(feat_set_t2, args.band_width_t2)

    neigh_idx_t1 = np.argsort(-dist_set_t1, axis=1)
    neigh_idx_t2 = np.argsort(-dist_set_t2, axis=1)

    fw_node_dist = np.zeros((obj_num, 1))
    bw_node_dist = np.zeros((obj_num, 1))

    for i in range(obj_num):
        fw_node_dist[i] = np.mean(np.abs(dist_set_t1[i, neigh_idx_t1[i, 1:args.knn_num]] - dist_set_t2[i, neigh_idx_t1[i, 1:args.knn_num]]))
        bw_node_dist[i] = np.mean(np.abs(dist_set_t2[i, neigh_idx_t2[i, 1:args.knn_num]] - dist_set_t1[i, neigh_idx_t2[i, 1:args.knn_num]]))
        # fw_node_dist[i] = np.mean(np.abs(dist_set_t1[i, neigh_idx_t1[i, 1:args.knn_num]] - dist_set_t1[i, neigh_idx_t2[i, 1:args.knn_num]]))
        # bw_node_dist[i] = np.mean(np.abs(dist_set_t2[i, neigh_idx_t1[i, 1:args.knn_num]] - dist_set_t2[i, neigh_idx_t2[i, 1:args.knn_num]]))

    diff_map = np.zeros((height, width))
    for i in range(obj_num):
        diff_map[objects == i] = fw_node_dist[i] + bw_node_dist[i]
    ...
    imageio.imsave('./result/Ver_' + str(time.time()) + '.png', bcm)
    diff_map = 255 * (diff_map - np.min(diff_map)) / (np.max(diff_map) - np.min(diff_map))
    imageio.imsave('./result/Ver_' + str(time.time()) + '_DI.png', diff_map.astype(np.uint8))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--band_width_t1', type=float, default=1, help='The bandwidth of Gaussian kernel when calculating the adj_mat of t1')
    parser.add_argument('--band_width_t2', type=float, default=0.7, help='The bandwidth of Gaussian kernel when calculating the adj_mat of t2')
    ...
    parser.add_argument('--deep_band_width_t2', type=float, default=0.15,
                        help='The bandwidth of the Gaussian kernel when calculating the adjacency matrix using deep vertex representations')
    parser.add_argument('--knn_num', type=int, default=50,
